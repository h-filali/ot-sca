...
from target.communication.sca_ml_dsa_commands import OTMLDSA
...
"""ML-DSA SCA capture script.

Captures power traces during different ML-DSA operations.

Typical usage:
>>> ./capture_ml_dsa.py -c configs/ml_dsa_sca_cw310.yaml -p projects/ml_dsa_sca_capture
"""
...
def establish_communication(target, capture_cfg: CaptureConfig):
    ...
    Returns:
        ot_ml_dsa: The communication interface to the ML-DSA SCA application.
        ot_prng:   The communication interface to the PRNG SCA application.
        ot_trig:   The communication interface to the SCA trigger.
    """
    # Create communication interface to OT ML-DSA.
    ot_ml_dsa = OTMLDSA(target=target, protocol=capture_cfg.protocol)
    ...
    return ot_ml_dsa, ot_prng, ot_trig
...
def capture(scope: Scope, ot_ml_dsa: OTMLDSA, ot_prng: OTPRNG,
    ...
    """ Capture power consumption during execution of ML-DSA SCA penetration tests.

    Supports the following captures:
    * ml_dsa.sca.ntt: Execute NTT on OTBN.
        ...
        while remaining_num_traces > 0:
            ...
            if "batch" in capture_cfg.test_mode:
                if "fvsr" in capture_cfg.test_mode:
                    ...
                    ot_ml_dsa.start_test(capture_cfg.test_mode, data_fixed, capture_cfg.num_segments)
                    ...
                    ot_ml_dsa.start_test(capture_cfg.test_mode, capture_cfg.num_segments)
            else:
...
def main(argv=None):
    ...
    ot_ml_dsa, ot_prng, ot_trig = establish_communication(target, capture_cfg)

    # Configure SW trigger.
    ot_trig.select_trigger(1)

    # Capture traces.
    device_id = capture(scope, ot_ml_dsa, ot_prng, capture_cfg, project, target)
